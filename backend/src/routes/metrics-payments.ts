...
import type { DateRangePreset, GatewayRate } from "@fbc/shared";
...
export function metricsPaymentsRoutes() {
  ...
  app.get("/", async (c) => {
    ...
    const [{ orders, truncated }, shopResp, prefs] = await Promise.all([
      fetchOrdersForRange(graphql, range),
      graphql<{ shop: { currencyCode: string } }>(SHOP_CURRENCY_QUERY),
      readMetafield<Preferences>(graphql, METAFIELD_KEYS.config),
    ]);
